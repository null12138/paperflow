...
# 每家出版社：机构登录入口 + 找 SSO 链接的 href 特征 + 可用 PDF 路径模板（用 {doi}/{suffix}）
PUBLISHER_SSO = {
    "wiley": {"login": "https://onlinelibrary.wiley.com/institutional-login",
               "sso": ("ssostart",), "pdfs": ("/doi/pdfdirect/{doi}", "/doi/pdf/{doi}")},
    "acs": {"login": "https://pubs.acs.org/page/4lib/institution.html",
             "sso": ("shibboleth", "institutional"), "pdfs": ("/doi/pdf/{doi}",)},
    "sciencedirect": {"login": "https://www.sciencedirect.com/user/institution/login",
                       "sso": ("shibboleth", "institutional"),
                       "pdfs": ("/science/article/pii/{suffix}/pdfft",
                                "/doi/pdfdirect/{doi}")},
    "springer": {"login": "https://wayf.springernature.com/?redirect_uri=https://link.springer.com",
                  "sso": ("shibboleth", "institutional-login"),
                  "pdfs": ("/content/pdf/{doi}.pdf",)},
    "nature": {"login": "https://www.nature.com/nature/",
                "sso": ("shibboleth", "institutional"),
                "pdfs": ("/articles/{suffix}.pdf",)},
    "tandfonline": {"login": "https://www.tandfonline.com/institutional-login",
                     "sso": ("ssostart", "shibboleth", "institutional"),
                     "pdfs": ("/doi/pdf/{doi}?needAccess=true",)},
    "ieee": {"login": "https://ieeexplore.ieee.org/servlet/wayf.jsp",
              "sso": ("shibboleth", "institutional"), "pdfs": ()},
    "oxford": {"login": "https://academic.oup.com/action/showLogin?targetURL=",
                "sso": ("shibboleth", "institutional"), "pdfs": ()},
    "royalsociety": {"login": "https://royalsocietypublishing.org/",
                      "sso": ("shibboleth", "institutional"), "pdfs": ()},
    "sage": {"login": "https://journals.sagepub.com/",
              "sso": ("shibboleth", "institutional"), "pdfs": ()},
    "asce": {"login": "https://ascelibrary.org/",
              "sso": ("shibboleth", "institutional"), "pdfs": ()},
}
...
# DOI 前缀 → 出版社（不依赖网络解析，稳健）
DOI_PUBLISHER = {
    "10.1021": "acs", "10.1022": "acs", "10.1016": "sciencedirect",
    "10.1111": "wiley", "10.1002": "wiley", "10.1093": "oxford",
    "10.1038": "nature", "10.1080": "tandfonline", "10.1109": "ieee",
    "10.1098": "royalsociety", "10.1177": "sage", "10.1061": "asce",
}


def detect_publisher(url_or_doi: str) -> str | None:
    """优先按 URL 主机判断，其次按 DOI 前缀映射。"""
    if url_or_doi.startswith("http"):
        host = (urllib.parse.urlparse(url_or_doi).hostname or "").lower()
        for dom, pub in _HOST_PUBLISHER:
            if dom in host:
                return pub
    for prefix, pub in DOI_PUBLISHER.items():
        if url_or_doi.casefold().startswith(prefix):
            return pub
    return None
...
class CarsiEngine:
    ...
    def fetch(self, doi: str, target: Path) -> tuple[bool, str]:
        ...
        if not publisher:
            return False, f"CARSI: 无法识别出版社 ({doi})"
    ...
    def _browser(self, doi: str, article_url: str, publisher: str,
        ...
        _p, browser, ctx = _auth._launch_browser(headful=True)
        try:
            page = ctx.new_page()
            if saved:
                try:
                ...
                if any(x in url for x in _AUTH_URL_KEYWORDS) or any(x in title for x in _AUTH_TITLES):
                    ...
                    for _ in range(100):
                        ...
                        try:
                            ...
                            break
                        is_auth = any(x in title for x in _AUTH_TITLES) or \
                                  any(x in url for x in _AUTH_URL_KEYWORDS)
                        if not is_auth and n_cookies > base_cookies + 2:
                            cookies = ctx.cookies()
...
def carsi_login(publisher: str, school: str) -> bool:
    """独立 CARSI 登录步骤：打开该出版社的机构登录页，走完整登录流程。"""
    from .. import auth as _auth

    cfg = PUBLISHER_SSO.get(publisher)
    if not cfg:
        print(f"未知出版社 {publisher}；可用: {', '.join(PUBLISHER_SSO)}")
        return False
    target_url = cfg.get("login") or "https://www.nature.com"
    try:
        _p, browser, ctx = _auth._launch_browser(headful=True)
        try:
            page = ctx.new_page()
            ...
            print(f"\n  [CARSI] 已打开 {school} 的机构认证页，请在浏览器完成 CAS 登录"
                  "（学号+密码/短信验证）…")
            # base 在进入等待前取样，排除 CF cookie 干扰
            time.sleep(3)
            base_cookies = len([c for c in ctx.cookies()
                                if "cloudflare.com" not in (c.get("domain") or "")])
            ok = False
            for _ in range(100):
                time.sleep(3)
                try:
                ...
                is_auth = any(x in title for x in _AUTH_TITLES) or \
                          any(x in url for x in _AUTH_URL_KEYWORDS)
                non_cf = [c for c in cookies
                          if "cloudflare.com" not in (c.get("domain") or "")]
                # 登录成功：离开登录页 且 出现实质性的非 CF 新 cookie
                if not is_auth and len(non_cf) > base_cookies + 2:
                    save_cookies(publisher, non_cf)
                    print(f"  ✓ CARSI 登录成功，已保存 {len(non_cf)} cookies")
                    ok = True
                    break
            if not ok:
                print("  ✗ 未检测到登录态（cookies 未变化）；窗口已保持，若已登录请重新运行")
            return ok
        finally:
            try:
                browser.close()
